...
echo "=== G5 (8x A10G) Benchmark Setup ==="
...
# 3. NVMe setup - g5.48xlarge has 2x local NVMe SSDs
# DLAMI typically mounts at /opt/dlami/nvme, verify
if mountpoint -q /opt/dlami/nvme 2>/dev/null; then
    echo "NVMe already mounted at /opt/dlami/nvme"
else
    echo "Setting up NVMe storage..."
    # Find NVMe devices (exclude root volume)
    NVME_DEVS=$(lsblk -dpno NAME | grep nvme | grep -v nvme0n1 || true)
    if [ -n "$NVME_DEVS" ]; then
        FIRST_DEV=$(echo "$NVME_DEVS" | head -1)
        sudo mkfs.ext4 -F "$FIRST_DEV"
        sudo mkdir -p /opt/dlami/nvme
        sudo mount "$FIRST_DEV" /opt/dlami/nvme
        sudo chown -R ubuntu:ubuntu /opt/dlami/nvme
    else
        echo "WARNING: No additional NVMe devices found, using EBS"
        sudo mkdir -p /opt/dlami/nvme
        sudo chown -R ubuntu:ubuntu /opt/dlami/nvme
    fi
fi
...
# A10G is Ampere (SM 8.6), use CUDA 12.4
...
# Install pluto dependencies (skip PyTorch reinstall)
pip install -e . || true
pip install pytorch-lightning==2.0.1 hydra-core==1.3.2 wandb l5kit shapely==2.0.1 || true
echo "=== PLUTO_DONE ==="

# 11. Apply Pluto code patches for BF16 compatibility
...
print(f'\\n=== A10G Specs ===')
print(f'VRAM per GPU: {props.total_mem / 1024**3:.1f} GB')
...
echo "  2. Run benchmark:  bash run_benchmark_g5.sh"
