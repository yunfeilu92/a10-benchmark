...
sample_dirs = []
# A10G: 使用 256 samples (32/卡) 以匹配训练 batch size
TARGET_SAMPLES = 256
for log_dir in sorted(os.listdir(cache_dir))[:20]:
    ...
    for scenario_type in os.listdir(log_path):
        ...
        for token in os.listdir(st_path):
            ...
            if len(sample_dirs) >= TARGET_SAMPLES:
                break
        if len(sample_dirs) >= TARGET_SAMPLES:
            break
    if len(sample_dirs) >= TARGET_SAMPLES:
...
# A10G specs
A10G_BF16_TFLOPS = 125  # A10G TF32 峰值，BF16 稀疏约 250 TFLOPS
A10G_FP32_TFLOPS = 31.2

print(f"\n{'='*60}")
print(f"=== A10G UTILIZATION ANALYSIS (10 forward, bs={bs}) ===")
print(f"{'='*60}")
print(f"\n--- Time Breakdown ---")
...
if total_cuda_us > 0 and total_flops > 0:
    ...
    print(f"A10G TF32 peak:       {A10G_BF16_TFLOPS} TFLOPS (per GPU)")
    print(f"A10G FP32 peak:       {A10G_FP32_TFLOPS} TFLOPS (per GPU)")
    print(f"Compute utilization:  {achieved/A10G_BF16_TFLOPS*100:.2f}% (vs TF32 peak)")

print(f"\n--- A10G vs H200/L40S Context ---")
print(f"A10G: 24GB VRAM, {A10G_BF16_TFLOPS} TFLOPS TF32, PCIe Gen4")
print(f"L40S: 48GB VRAM, 362 TFLOPS BF16, PCIe Gen4")
print(f"H200: 80GB VRAM, 1979 TFLOPS BF16, NVLink")
print(f"\n--- Why A10G utilization matters ---")
print(f"1. Model size: {total_params:,} params = {total_params*2/1024/1024:.1f} MB (tiny)")
print(f"2. GEMM dimensions: 128x128 (dim=128), small for Tensor Cores")
print(f"3. MatMul accounts for only {matmul_time/total_cuda_us*100:.1f}% of GPU time")
print(f"4. A10G has fewer SMs than H200/L40S, potentially better utilization for small models")
print(f"5. Kernel count: {sum(1 for e in events if e.cuda_time > 0)} unique kernels in one forward pass")
